...
async function runUser(browser,i){
  ...
  const done=(async()=>{
    try{
      ...
      else { await p.goto(`${base}/videos/index.html`,{waitUntil:'domcontentloaded'}); await p.waitForSelector('.v-card',{timeout:15000}); await Promise.all([p.waitForNavigation({waitUntil:'domcontentloaded',timeout:15000}), p.click('.v-card')]); await p.waitForSelector('#cmt-body',{timeout:12000}); await p.fill('#cmt-body','clip '+i); await p.click('#cmt-post'); await p.waitForTimeout(400); }
      sessionsOK++;
