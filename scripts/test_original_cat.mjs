...
try{
  ...
  const want=["Mochi's Treat Catch","Mochi Says","Perfect Fetch","Word Rescue","Paw Reflex","Mochi Quiz","Mochi Match","Whack-a-Mochi","Mochi Slide"];
  const allShown=want.every(w=>names.some(n=>n.includes(w)));
  (cnt===9 && allShown) ? pass('Original filter shows exactly the 9 originals') : fail('got '+cnt+' cards: '+names.join(', '));
