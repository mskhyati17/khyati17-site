...
try{
  ...
  const want=["Mochi's Treat Catch","Mochi Says","Perfect Fetch","Word Rescue","Paw Reflex","Mochi Quiz","Mochi Match","Whack-a-Mochi","Mochi Slide","Mochi Type","Mochi 2048","Mochi Flap","Mochi Maze"];
  const allShown=want.every(w=>names.some(n=>n.includes(w)));
  (cnt===13 && allShown) ? pass('Original filter shows exactly the 13 originals') : fail('got '+cnt+' cards: '+names.join(', '));
