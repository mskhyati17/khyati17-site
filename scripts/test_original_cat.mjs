...
try{
  ...
  const want=["Mochi's Treat Catch","Mochi Says","Perfect Fetch","Word Rescue","Paw Reflex","Mochi Quiz"];
  const allShown=want.every(w=>names.some(n=>n.includes(w)));
  (cnt===6 && allShown) ? pass('Original filter shows exactly the 6 originals') : fail('got '+cnt+' cards: '+names.join(', '));
